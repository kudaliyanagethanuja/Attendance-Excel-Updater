...
def format_time_for_display(time_val):
    """
    Format time as HH:MM for display/logging only.
    This is NOT saved to Excel.
    """
    if pd.isna(time_val) or time_val == '':
        return ''
    try:
        time_obj = pd.to_datetime(str(time_val)).time()
        return time_obj.strftime('%H:%M')  # No seconds
    except:
        return str(time_val)

def to_excel_time_only(val):
    """
    Convert to string HH:MM for Excel so it shows exactly like 06:38 or 15:15.
    """
    try:
        if pd.isna(val) or val == '':
            return None
        dt = pd.to_datetime(val)
        return dt.strftime('%H:%M')  # Directly store as HH:MM string
    except:
        return None
...
def update_attendance_file(input_file, output_file):
    ...
    for sheet_name, df in excel_data.items():
        ...
        try:
            ...
            # Store as HH:mm strings
            merged_df['First Check In'] = merged_df['First Check In'].apply(to_excel_time_only)
            merged_df['Last Check Out'] = merged_df['Last Check Out'].apply(to_excel_time_only)
...
def login():
    if request.method == 'POST':
        ...
        if username == 'admin' and password == 'software@123':
